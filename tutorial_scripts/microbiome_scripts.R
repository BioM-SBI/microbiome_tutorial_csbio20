...
############################################################################################################
# Functions for microbiome analysis, mainly for formatting data from mothur resulting files for visualization in R
############################################################################################################

# Function for formatting mothur rarefaction data for rarefaction plot using R
import_mothur_rarefaction = function(file){
  # Import data to R dataframe
  data = read.table(file, header = T)
  
  # Select the column of mean values (not hci or lci)
  data = data %>% select('numsampled', matches("^X"))
  
  # Find and replace "X0.03." at the beginning of the colnames with blank
  names = str_replace_all(colnames(data), '^X0.03.','')
  colnames(data) = names
  
  # Merge matrix format into tabular format
  transformed_tbl <- melt(data, id.vars = 'numsampled')
  return(transformed_tbl)
}

# Function for importing abundance table from mothur
import_sharedTable <- function(input_shared){
	...
	# Import data to R data.frame 
	shared = read.table(shared_file, header=TRUE)
	# Remove first column $label
	shared = shared[-1]
	# Remove second column $numOtus
	shared = shared[-2]

	# Transpose dataframe to have features on rows and sampleID on columns
...
# Function for importing taxonomy file from mothur
import_taxonomyFile <- function(input_taxonomy){
	### ---------------------------------------------------------------------------- #####
	### This is a function to import taxonomy annotation (mothur.cons.taxonomy) file #####
	### Remove all confident values in parenthesis (xxx) of taxonomic lineage in 3rd column####
	### which xxx represents 0 -> 100
	### Remove semicolon (;) at the end of string in 3rd column
	...
	# Create the new dataframe to get the taxonomic infomation at each rank
	taxRank = c('Domain','Phylum','Class','Order','Family','Genus')
	
	# Remove confident number at the end of taxa name, 
	# for example: 
	...
	# Split Taxonomic lineage into individual rank
...
combine_df <- function(transposeTbl, rank_anno){
	...
	### Group the data by phylum and summarize values
...
filter_dominance <- function(tbl, cutoff){
	### ------------------------------------------------------------------------------------ ####
	### This function calculates relative abundance and filter dominant features (OTU or ASV) ###
	### ------------------------------------------------------------------------------------ ####
	...
	# Calculate each maximum abundance (from all samples) of each taxon
...
# Calculate non-top N taxa as 'others' to the calculated profiles
...
# Create dominant taxa annotation for dominant features (OTU / ASV)
...
# Function for importing mothur taxonomy file 
